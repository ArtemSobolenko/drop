...
public class Main implements ApplicationListener {
    ...
    private void input() {
        ...
        if (Gdx.input.isKeyPressed(Input.Keys.SHIFT_LEFT)) {
            speed *= 2;
            speedSound.play(0.1f, 1.2f, -0.5f);
        }
        ...
        if (Gdx.input.isTouched()) {
           ...
           // bucketSprite.setPosition(touchPos.x - 0.5f, touchPos.y - 0.5f);
        }
    ...
    private void logic() {
      ...
      //  bucketSprite.setX(MathUtils.clamp(bucketSprite.getX(), -0.5f, +0.5f + worldWidth - bucketSprite.getWidth()));
        ...
        for (int i = dropSprites.size - 1; i >= 0; i--) {
            ...
            float dropWith = dropSprite.getWidth();
            float dropHeight = dropSprite.getHeight();

            dropSprite.translateY((-2f * delta));

            dropRectangle.set(dropSprite.getX(), dropSprite.getY(), dropWith, dropHeight);
